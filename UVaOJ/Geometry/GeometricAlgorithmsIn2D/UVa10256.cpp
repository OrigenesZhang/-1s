...
const int maxn=511;
Point P[maxn], ch1[maxn], Q[maxn], ch2[maxn];
int M, C;
int main(){
	while(scanf("%d%d", &M, &C)==2){
		...
		for(int i=0; i<C; i++)
			Q[i].read();
		int n1=Polygonal::convexHull(P, M, ch1);
		int n2=Polygonal::convexHull(Q, C, ch2);
		Polygonal::doesConvexHullConvexHullIntersect(ch1, n1, ch2, n2)?puts("No"):puts("Yes");
	}
	return 0;
}
