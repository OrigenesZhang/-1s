...
bool onAnySegment(Point P){
	for(int i=0; i<n; i++)
		if(Linear::onSegment(P, p1[i], p2[i])) return true;
	return false;
}
bool intersectWithAnySegment(Point a, Point b){
	Point o;
	for(int i=0; i<n; i++)
		if(Linear::getSegSegIntersection(a, b, p1[i], p2[i], o)) return true;
	return false;
}
bool dfs(int u){
	if(u==1) return true;
...
int main(){
	while(scanf("%d", &n)==1){
		...
		for(int i=0; i<n; i++){
			...
			a.read(), b.read();
			Vector V=b-a;
			V=V/Vectorial::getLength(V);
			p1[i]=a-V*delta;
			p2[i]=b+V*delta;
		}
		vector<Point> vertices={{0, 0}, {1e5, 1e5}};
		for(int i=0; i<n; i++){
