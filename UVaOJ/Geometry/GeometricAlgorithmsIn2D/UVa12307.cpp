...
pdd solve(Point *p, int n, Point *ch){
	int m=Polygonal::convexHull(p, n, ch);
	double area=1e15, perimeter=1e15;
	int t=1, l=1, r=1;
	for(int i=0; i<m; i++){
		while(dcmp(Vectorial::getCross(ch[Polygonal::Next(i, m)]-ch[i], ch[Polygonal::Next(t, m)]-ch[t]))==1)
			t=Polygonal::Next(t, m);
		while(dcmp(Vectorial::getDot(ch[Polygonal::Next(i, m)]-ch[i], ch[Polygonal::Next(r, m)]-ch[r]))==1)
			r=Polygonal::Next(r, m);
		if(!i) l=Polygonal::Next(r, m);
		while(dcmp(Vectorial::getDot(ch[Polygonal::Next(i, m)]-ch[i], ch[Polygonal::Next(l, m)]-ch[l]))==-1)
			l=Polygonal::Next(l, m);
		double d=Vectorial::getLength(ch[Polygonal::Next(i, m)]-ch[i]);
		double h=Vectorial::getCross(ch[Polygonal::Next(i, m)]-ch[i], ch[t]-ch[i])/d;
		double w=Vectorial::getDot(ch[Polygonal::Next(i, m)]-ch[i], ch[r]-ch[l])/d;
