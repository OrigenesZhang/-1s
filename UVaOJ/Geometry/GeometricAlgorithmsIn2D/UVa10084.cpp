...
const int maxn=55;
DirLine L[maxn]={{{0, 0}, {10, 0}}, {{10, 0}, {0, 10}}, {{10, 10}, {-10, 0}}, {{0, 10}, {0, -10}}};
Point ch[maxn], cur, pre;
char op[maxn];
bool flag;
int n=4;
int main(){
	cur={0, 0};
	while(scanf("%lf%lf%s", &pre.x, &pre.y, op)==3){
		swap(cur, pre);
		if(flag){
			puts("0.00");
			continue;
		...
		Point M=(cur+pre)*0.5;
		Vector norm=Vectorial::getNormal(cur-pre);
		if(op[0]=='H') norm=norm*(-1);
		L[n++]={M, norm};
		int m=Polygonal::halfPlaneIntersection(L, n, ch);
		printf("%.2lf\n", Polygonal::getArea(ch, m));
	}
	return 0;
}
