...
int main(){
	while(scanf("%d", &n)==1){
		...
		for(int s=0; s<(1<<n); s++){
			...
			double len=0;
			for(int i=0; i<n; i++){
				if(s&(1<<i)) pts.push_back(p[i]);
				else{
					cost+=v[i];
					len+=l[i];
				}
			}
			if(cost>ans||(cost==ans&&n-pts.size()>=cnt)) continue;
			ch=Polygonal::convexHull(pts);
			int m=ch.size();
//			printf("----------%d------%lf\n", s, len);
//			for(int i=0; i<m; i++)
//				ch[i].write();
			for(int i=0; i<m; i++)
				len-=Vectorial::getLength(ch[i]-ch[Polygonal::Next(i, m)]);
//			printf("----------%d------%lf\n", s, len);
//			puts("");
			if(dcmp(len)<0) continue;
			ans=cost;
			cnt=n-pts.size();
			rs=s;
			rem=len;
