//居然出现了 virtual memory exhausted 这个报错orz
#include <cstdio>
#include <cmath>
#include <cstring>
#include <functional>
...
void cut(int x1, int y1, int x2, int y2){
	new_pieces.clear();
	for(auto it:pieces){
		Polygon lft=Polygonal::cutPolygon(it, {x1, y1}, {x2, y2});
		Polygon rgt=Polygonal::cutPolygon(it, {x2, y2}, {x1, y1});
		if(lft.size()>=3) new_pieces.push_back(lft);
		if(rgt.size()>=3) new_pieces.push_back(rgt);
	}
	pieces=new_pieces;
}
bool discIntersectPolygon(Polygon poly, Point p, double R){
	if(Polygonal::isPointInPolygon(p, poly)) return true;
	if(Circular::PointCircleRelation(poly[0], {p, R})==-1) return true;
	int n=poly.size();
	for(int i=0; i<n; i++){
		if(Circular::haveCircleSegmentIntersection(poly[i], poly[Polygonal::Next(i, n)], {p, R}))
			return true;
		if(Circular::PointCircleRelation((poly[i]+poly[Polygonal::Next(i, n)])*0.5, {p, R})==-1)
			return true;
	}
	return false;
}
int main(){
	...
	while(scanf("%d%d%d%d", &n, &m, &L, &W)==4){
		...
		for(int i=0; i<m; i++){
			...
			vector<double> ans;
			for(auto it:pieces)
				if(discIntersectPolygon(it, {x, y}, R))
					ans.push_back(Polygonal::getArea(it));
			printf("%u", ans.size());
