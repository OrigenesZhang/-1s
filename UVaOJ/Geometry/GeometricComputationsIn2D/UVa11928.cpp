...
int main(){
	while(scanf("%d", &N)==1){
		...
		double rad=0;
		for(int i=0; i<N; i++){
			if(Linear::onSegment(O, P[i], P[i+1])||P[i]==O||P[i+1]==O){
				flag=false;
				break;
			}
			Vector u=P[i]-O, v=P[i+1]-O;
			double theta=Vectorial::getAngle(u, v);
			if(dcmp(Vectorial::getCross(u, v))<0) rad-=theta;
			else rad+=theta;
		}
		if(!flag){
			puts("Ouch!");
		...
		if(dcmp(rad)<0) printf("-%0.lf\n", fabs(rad)/(2*pi));
		else if(dcmp(rad)==0) puts("0");
		else printf("+%0.lf\n", fabs(rad)/(2*pi));
	}
	return 0;
}
