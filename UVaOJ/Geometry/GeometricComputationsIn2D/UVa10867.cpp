...
int main(){
	while(scanf("%d%d", &n, &m)){
		...
		while(m--){
			...
			vector<Point> s;
			for(int i=0; i<n; i++){
				if(Linear::getIntersection(A, B-A, poly[i], poly[Polygonal::Next(i, n)]-poly[i], t)){
					if(Linear::onSegmentInc(t, poly[i], poly[Polygonal::Next(i, n)]))
						s.push_back(t);
				}else if(dcmp(Vectorial::getCross(A-B, A-poly[i]))==0&&dcmp(Vectorial::getCross(A-B, A-poly[Polygonal::Next(i, n)]))==0){
					s.push_back(poly[i]);
					s.push_back(poly[Polygonal::Next(i, n)]);
				}
			}
			sort(s.begin(), s.end());
			double ans=0;
			for(int i=1; i<s.size(); i++){
				Point k=(s[i-1]+s[i])*0.5;
				if(Polygonal::isPointInPolygon(k, poly, n))
					ans+=Vectorial::getLength(s[i]-s[i-1]);
			}
			printf("%.3lf\n", ans);
		}
	}
