...
bool judge(double r){
	...
	Circle C(o, r);
	for(int i=1; i<=n; i++){
		int f1=Circular::PointCircleRelation(p[i-1], C);
		int f2=Circular::PointCircleRelation(p[i], C);
		if(f1<=0&&f2<=0){
			s+=Triangular::getArea(p[i-1], p[i], o);
			continue;
		}
		if(f1+f2==1){
			s+=Circular::getSectorArea(p[i-1], p[i], C);
			continue;
		}
		vector<Point> inter;
		double t1, t2;
		Circular::getLineCircleIntersection(p[i-1], p[i], C, t1, t2, inter);
		if(f1==-1&&f2==1){
			if(Linear::onSegment(inter[0], p[i-1], p[i])){
				s+=Circular::getSectorArea(p[i], inter[0], C);
				//A += Angle(v2,inter[0]);
				s+=Triangular::getArea(o, p[i-1], inter[0]);
			}else if(Linear::onSegment(inter[1], p[i-1], p[i])){
				s+=Circular::getSectorArea(p[i], inter[1], C);
				s+=Triangular::getArea(o, p[i-1], inter[1]);
			};
			continue;
		}
		if(f1==1&&f2==-1){
			if(Linear::onSegment(inter[0], p[i-1], p[i])){
				s+=Circular::getSectorArea(p[i-1], inter[0], C);
				s+=Triangular::getArea(o, p[i], inter[0]);
			}else if(Linear::onSegment(inter[1], p[i-1], p[i])){
				s+=Circular::getSectorArea(p[i-1], inter[1], C);
				s+=Triangular::getArea(o, p[i], inter[1]);
			}
			continue;
		}
		if(inter.size()<2) s+=Circular::getSectorArea(p[i-1], p[i], C);
		else if(inter.size()==2){
			if(dcmp(t1)>0&&dcmp(t1-1)<0){
				if(dcmp(t2)>0&&dcmp(t2-1)<0){
					s+=Circular::getSectorArea(p[i-1], p[i], C)-Circular::getSectorArea(inter[0], inter[1], C);
					s+=Triangular::getArea(o, inter[0], inter[1]);
				}else if(f1==1&&f2==1) continue;
			}else s+=Circular::getSectorArea(p[i-1], p[i], C);
		}
	}
	return dcmp(s-R)<0;
}
int main(){
	while(scanf("%d", &n)==1){
		...
		for(int i=0; i<n; i++){
			p[i].read();
			r=max(Vectorial::getLength(p[i]), r);
		}
		//printf("%lf\n", Polygonal::getArea(p, n));
		p[n]=p[0];
		for(int i=0; i<100; i++){
			double M=(l+r)/2;
			if(judge(M)) l=M;
