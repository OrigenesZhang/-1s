...
namespace Linear{
	...
	bool getSegSegIntersectionInc(Point& a1, Point& a2, Point& b1, Point& b2, Point& o){
		...
		if(onSegment(o, a1, a2)&&onSegment(o, b1, b2)) return true;
		return false;
	}
	/* 线段和直线的交点, 端点计入 */
...
void init(){
	...
	for(int i=1; i<=N; i++){
		s[i]={0};
		Vector v=segs[i][1]-segs[i][0];
		s[i].insert(Vectorial::getLength(v));
	}
}
bool judge(Point A, Point B, int pos){
	Vector v1=A-segs[pos][0], v2=B-segs[pos][0];
	double len1=Vectorial::getLength(v1);
	double len2=Vectorial::getLength(v2);
	if(len1>len2) swap(len1, len2);
	auto it=s[pos].upper_bound(len1);
	while(dcmp(*it-len1)==0) it++;
	if(dcmp(len2-(*it))>0) return false;
	return true;
}
int main(){
	scanf("%d", &T);
	for(int cs=1; cs<=T; cs++){
		...
		for(int i=1; i<N; i++){
			for(int j=i+1; j<=N; j++){
				Point inter;
				if(Linear::getSegSegIntersectionInc(segs[i][0], segs[i][1], segs[j][0], segs[j][1], inter)){
					flags[i][j]=true;
					p[i][j]=inter;
					Vector v=inter-segs[i][0];
					s[i].insert(Vectorial::getLength(v));
					v=inter-segs[j][0];
					s[j].insert(Vectorial::getLength(v));
				}
			}
		}
		int res=0;
			...
			for(int j=i+1; j<=N; j++){
				...
				for(int k=j+1; k<=N; k++){
					if(flags[i][k]&&flags[j][k]){
						if(p[i][j]==p[i][k]||p[i][k]==p[j][k]||p[j][k]==p[i][j]) continue;
						if(judge(p[i][j], p[i][k], i)&&judge(p[i][j], p[j][k], j)&&judge(p[j][k], p[i][k], k))
							res++;
					}
				}
			}
