...
namespace Linear{
	...
	bool isParalell(Point& p1, Point& p2, Line& L){
		Vector v1=p2-p1, v2(L.b, L.a);
		Vector n=getNormal(v1);
		return dcmp(getDot(n, v2))==0;
	}
	/* 点p到直线ab的距离 */
...
double minDist(int pos, double l, double r){
	for(int i=0; i<err_ctrl; i++){
		double m1=(2*l+r)/3; Point M1(m1, 0);
		double m2=(l+2*r)/3; Point M2(m2, 0);
		double d1=Linear::getDistanceToSegment(M1, segs[pos][0], segs[pos][1]);
		double d2=Linear::getDistanceToSegment(M2, segs[pos][0], segs[pos][1]);
		d1<d2?r=m2:l=m1;
	}
	return l;
}
double bs1(int pos, double l, double r, double R){
	for(int i=0; i<err_ctrl; i++){
		double m=(l+r)/2; Point M(m, 0);
		if(Linear::getDistanceToSegment(M, segs[pos][0], segs[pos][1])>R) l=m;
		else r=m;
	}
	return l;
}
double bs2(int pos, double l, double r, double R){
	for(int i=0; i<err_ctrl; i++){
		double m=(l+r)/2; Point M(m, 0);
		if(Linear::getDistanceToSegment(M, segs[pos][0], segs[pos][1])>R) r=m;
		else l=m;
	}
	return r;
}
bool judge(double r){
	vec.clear();
	for(int i=0; i<N; i++){
		double d=minDist(i, 0, L); Point D(d, 0);
		if(dcmp(Linear::getDistanceToSegment(D, segs[i][0], segs[i][1])-r)>=0){
		...
		D={0, 0};
		if(dcmp(Linear::getDistanceToSegment(D, segs[i][0], segs[i][1])-r)>=0){
			double dis=bs1(i, 0, d, r);
			vec.push_back(make_pair(0, 1));
			vec.push_back(make_pair(dis, -1));
		}
		D={L, 0};
		if(dcmp(Linear::getDistanceToSegment(D, segs[i][0], segs[i][1])-r)>=0){
			double dis=bs2(i, d, L, r);
			vec.push_back(make_pair(dis, 1));
			vec.push_back(make_pair(L, -1));
		}
	}
