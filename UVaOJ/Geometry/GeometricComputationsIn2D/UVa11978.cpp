...
int n, T;
double A;
double commonArea(Circle& C, Point A, Point B){
	if(A==B) return 0;
	if(A==C.o||B==C.o) return 0;
	Vector tmp1=A-C.o, tmp2=B-C.o;
	double OA=Vectorial::getLength(tmp1), OB=Vectorial::getLength(tmp2);
	double d=Linear::getDistanceToLine(o, A, B);
	int sg=dcmp(Vectorial::getCross(A, B));
	if(sg==0) return 0;
	double angle=Vectorial::getAngle(A, B);
	if(dcmp(OA-C.r)<=0&&dcmp(OB-C.r)<=0)
		return Vectorial::getCross(A, B)/2;
	else if(dcmp(OA-C.r)>=0&&dcmp(OB-C.r)>=0&&dcmp(d-C.r)>=0)
		return sg*C.r*C.r*angle/2;
	else if(dcmp(OA-C.r)>=0&&dcmp(OB-C.r)>=0&&dcmp(d-C.r)<0){
		Point prj=Linear::getPointToLine(o, A, B);
		if(Linear::onSegment(prj, A, B)){
			vector<Point> p;
			double t1, t2;
			Circular::getLineCircleIntersection(A, B, C, t1, t2, p);
			double s1=C.r*C.r*angle/2;
			double s2=C.r*C.r*Vectorial::getAngle(p[0], p[1])/2;
			s2-=fabs(Vectorial::getCross(p[0], p[1])/2);
			s1-=s2;
			return sg*s1;
		}else return sg*C.r*C.r*angle/2;
	}else{
		if(dcmp(OB-C.r)<0) swap(A, B);
		Point inter_point;
		double t1, t2;
		vector<Point> inter;
		Circular::getLineCircleIntersection(A, B, C, t1, t2, inter);
		if(Linear::onSegment(inter[0], A, B)) inter_point=inter[0];
		else inter_point=inter[1];
		double s=fabs(Vectorial::getCross(inter_point, A)/2);
		s+=C.r*C.r*Vectorial::getAngle(inter_point, B)/2;
		return s*sg;
	}
}
bool judge(double r){
