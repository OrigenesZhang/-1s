...
struct PSLG{
	...
	void build(){
		for(int u=0; u<n; u++){
			...
			sort(G[u].begin(), G[u].end(), bind(&cmp, this, _1, _2)); //这个骚操作UVa不让我过编译好气啊
#endif
#ifdef ONLINE_JUDGE
			for(int i=0; i<d; i++)
				for(int j=i+1; j<d; j++)
					if(edges[G[u][i]].ang>edges[G[u][j]].ang) swap(G[u][i], G[u][j]); //懒惰的我敲了一个冒泡排序
#endif
			for(int i=0; i<d; i++)
...
PSLG g;
int n, c;
Point P[maxp], V[maxp*maxp];
inline int getID(Point p){
	return int(lower_bound(V, V+c, p)-V);
}
Polygon simplify(const Polygon& poly){  //去重
	Polygon ans;
	int n=poly.size();
	for(int i=0; i<n; i++){
		Point a=poly[i];
		Point b=poly[(i+1)%n];
		Point c=poly[(i+2)%n];
		if(dcmp(Vectorial::getCross(a-b, c-b)))
			ans.push_back(b);
	}
	return ans;
}
int main(){
	while(scanf("%d", &n)==1){
		...
		for(int i=0; i<n; i++){
			for(int j=i+1; j<n; j++){
				Point o;
				if(Linear::getSegSegIntersection(P[i], P[Polygonal::Next(i, n)], P[j], P[Polygonal::Next(j, n)], o)){
					V[c++]=o;
					dist[i].push_back(Vectorial::getLength(o-P[i]));
					dist[j].push_back(Vectorial::getLength(o-P[j]));
				}
			}
		}
		sort(V, V+c);
		c=int(unique(V, V+c)-V);  //可以放大坐标范围转int去重, 牺牲精度换取鲁棒性和速度
		g.init(c);
		for(int i=0; i<c; i++){
			g.x[i]=V[i].x;
			g.y[i]=V[i].y;
		...
		for(int i=0; i<n; i++){
			Vector v=P[Polygonal::Next(i, n)]-P[i];
			double len=Vectorial::getLength(v);
			dist[i].push_back(0);
