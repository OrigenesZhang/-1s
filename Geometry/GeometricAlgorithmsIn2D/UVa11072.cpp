...
namespace Polygonal{
	...
	int isPointInPolygon(Point& p, Point *poly, int n){
		int wn=0;
		for(int i=0; i<n; i++){
			if(Linear::onSegmentInc(p, poly[i], poly[Next(i, n)])) return -1;
...
int p, r;
Point pts[maxn], ch[maxn];
Point P;
int main(){
	while(scanf("%d", &p)==1){
		for(int i=0; i<p; i++)
			pts[i].read();
		int n=Polygonal::convexHull(pts, p, ch);
		scanf("%d", &r);
		while(r--){
			P.read();
			if(Polygonal::isPointInPolygon(P, ch, n)) puts("inside");
			else puts("outside");
		}
	}
	return 0;
