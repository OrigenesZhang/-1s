...
int main(){
	while(scanf("%d", &n1)==1){
		if(n1<3) break;
		for(int i=0; i<n1; i++) p1[i].read();
		scanf("%d", &n2);
		for(int i=0; i<n2; i++) p2[i].read();
		inter.clear();
		for(int i=0; i<n1; i++){
			for(int j=0; j<n2; j++){
				Point o;
				bool flag=Linear::getSegSegIntersectionInc(p1[i], p1[Polygonal::Next(i, n1)], p2[j], p2[Polygonal::Next(j, n2)], o);
				if(flag) inter.push_back(o);
			}
		}
		for(int i=0; i<n1; i++)
			if(Polygonal::isPointInPolygon(p1[i], p2, n2))
				inter.push_back(p1[i]);
		for(int i=0; i<n2; i++)
			if(Polygonal::isPointInPolygon(p2[i], p1, n1))
				inter.push_back(p2[i]);
		sort(inter.begin(), inter.end());
		auto ed=unique(inter.begin(), inter.end());
		printf("%d\n", distance(inter.begin(), ed));
		for(auto it=inter.begin(); it!=ed; ++it)
			printf("%.2lf %.2lf\n", it->x, it->y);
	}
	return 0;
}
