//这题的题意比较恶心
//在警察或者匪徒的连线上一般算但是如果没有其他警察/匪徒的话不算
//WA了两下&&代码写得比较乱
#include <cstdio>
#include <cmath>
#include <vector>
...
void getPolarCoordinate(Point& P, Point& O, double& rad){
	Vector v=P-O;
	rad=Vectorial::getAngle(v);
}
const int maxn=234;
Point C[maxn], R[maxn], P;
double p[maxn];
int c, r, o, cs=1;
int main(){
	while(scanf("%d%d%d", &c, &r, &o)==3){
		if(!c&&!r&&!o) break;
		for(int i=0; i<c; i++) C[i].read();
		for(int i=0; i<r; i++) R[i].read();
		printf("Data set %d:\n", cs++);
		while(o--){
			P.read();
			bool skip=false;
			for(int i=0; i<c; i++){
				getPolarCoordinate(C[i], P, p[i]);
				if(C[i]==P&&c>=3){
			...
			if(c<=2) flag=false;
			for(int i=1; i<=c; i++){
				if(dcmp(p[i]-p[i-1]-pi)<=0) continue;
				flag=false;
				break;
			}
			if(flag){
