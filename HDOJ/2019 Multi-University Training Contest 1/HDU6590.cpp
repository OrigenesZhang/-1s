...
Point P[maxn], Q[maxn], ch1[maxn], ch2[maxn];
...
void solve() {
  ...
  while (n--) {
    int x1, x2, y;
    scanf("%d%d%d", &x1, &x2, &y);
    if (y == -1) P[p++] = Point(x1, x2);
    else Q[q++] = Point(x1, x2);
  }
  int n1 = Polygonal::convexHull(P, p, ch1);
  int n2 = Polygonal::convexHull(Q, q, ch2);
  puts(!n1 || !n2 || !Polygonal::doesConvexHullConvexHullIntersect(ch1, n1, ch2, n2) ? "Successful!" : "Infinite loop!");
}
